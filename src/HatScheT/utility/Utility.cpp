...
//		return minRecII;
    ...
    double Utility::calcResMII(ResourceModel *rm, Target *t) {
        ...
        if (t == nullptr) {
            for (auto it = rm->resourcesBegin(); it != rm->resourcesEnd(); ++it) {
                ...
                double tempMax =
                        ((double) opsUsingR) /
                        ((double) avSlots); // + (double)(opsUsingR % avSlots != 0); < -- ?? patrick
    ...
    int Utility::calcAbsolutMaxII(Graph *g, ResourceModel *rm) {
        ...
        if (g->getNumberOfVertices() > 200) {
            ...
            if (criticalPath <= 0) {
                HatScheT::ASAPILPScheduler *asapilp = new HatScheT::ASAPILPScheduler(*g, *rm,
                                                                                     {"CPLEX", "Gurobi", "SCIP"});
    ...
    double Utility::calcRecMII(Graph *g, ResourceModel *rm) {
        ...
        return max(1.0,
                   (double) (solver.getResult().objectiveValue)); // RecMII could be 0 if instance has no backedges.
#else
...
//#endif
    ...
    bool
    Utility::resourceAvailable(std::map<Vertex *, int> &startTimes, ResourceModel *rm, const Resource *r,
                               Vertex *checkV,
                               int timeStep) {
        //unlimited
    ...
    Binding::BindingContainer Utility::convertBindingContainer(Graph *g, ResourceModel *rm, const int &II,
        ...
        auto checkConnection = [](
                ...
                const std::string &srcRes, const int &srcFU, const int &srcPort, const std::string &dstRes,
                const int &dstFU,
                const int &dstPort) {
            for (auto &c: *conn) {
        ...
        for (auto &it: regChainLength) {
            ...
            for (int i = 0; i < numRegs - 1; i++) {
                 ...
                 */
                auto *cRegReg = checkConnection(&b.connections, "register", regOffset + i, 0, "register",
                                                regOffset + i + 1, 0);
                /*
        ...
        for (auto &connection: bChain.fuConnections) {
            ...
            if (numRegs > 0) {
                ...
                auto *cRegFU = checkConnection(&b.connections, "register", srcRegisterIndex, 0, rDstName, fuDstIndex,
                                               dstPort);
        ...
        for (auto &connection: bChain.fuConnections) {
            ...
            auto numRegs = connection.second.first;
            auto dstPort = connection.second.second;
            std::cout << "  '" << rSrcName << "' (" << fuSrcIndex << ") -> '" << rDstName << "' (" << fuDstIndex
                      << ") port "
        ...
        for (auto &vStart: g->Vertices()) {
            ...
            while (!stack.empty()) {
                ...
                for (auto &it: path) {
                    ...
                    std::cout << "found loop with q = " << q << " = " << std::get<2>(it) << "/" << std::get<1>(it)
                              << std::endl;
                    if (q > minII) {
                        minII = q;
                    }
                ...
                for (auto &e: g->Edges()) {
                    ...
                    std::tuple<Vertex *, Vertex *, double, double> nextStackElement{vDst, vSrc, edgeDistance,
                                                                                    edgeDelay};
        ...
        for (auto &e: g->Edges()) {
            ...
            for (int sDst = 0; sDst < samples; sDst++) {
                ...
                if (bSrc >= resourceLimits.at(rSrc)) {
                    ...
                    std::cout << "Detected invalid binding for vertex '" << vSrc->getName() << "': FU index is '"
                              << bSrc
    ...
    void
    Utility::unroll(Graph *gUnroll, ResourceModel *rmUnroll, const int &numSamples, const int &cycleLength, Graph *g,
    ...
    int Utility::calcMaxLatencyEstimation(Graph *g, ResourceModel *rm,
        ...
        if (!quiet) {
            cout << "Max Latency Suggestion: " << helper << " + " << length << " = " << helper + length << endl;
        }
...
#else
        throw (HatScheT::Exception("Utility::calcMaxIIWithSMT: Z3 has to be linked to use this function."));
        return -1;
#endif
    }
}
