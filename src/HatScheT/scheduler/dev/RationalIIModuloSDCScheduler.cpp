...
namespace HatScheT {
	...
	RationalIIModuloSDCScheduler::RationalIIModuloSDCScheduler(Graph &g, ResourceModel &resourceModel,std::list<std::string> solverWishlist) :
	...
	void RationalIIModuloSDCScheduler::scheduleIteration() {
        cout << "sample: " << this->samples << " modulo: "<< this->modulo << endl;
        ...
        RationalIIModuloSDCScheduler::setMRT(this->mrt,this->resourceModel,this->initiationIntervals,this->samples,this->modulo,this->quiet);
    ...
    void RationalIIModuloSDCScheduler::createDataDependencyConstraints() {
        for (auto it = this->g.edgesBegin(); it != this->g.edgesEnd(); it++) {
            ...
            ScaLP::Constraint c = (
                    ...
                    this->resourceModel.getResource(&src)->getLatency()-edge->getDelay());
            *this->solver << c;
        }
    }
    ...
    void RationalIIModuloSDCScheduler::backtracking(Vertex *I, const int &time) {
        ...
        for (auto it : evictInst) {
            ...
            cout << "Back into Queue: "<< it->getName() << " Resource Conflict"<< endl;
            PriorityHandler1::putIntoSchedQueue(it, this->pType, &this->priorityForSchedQueue, &this->schedQueue);
        }
        ///////////////////////
        // ALGORITHM LINE 16 //
        ...
        if (this->dependencyConflict(I, evictTime)) {
            ...
            for (auto it : copy) {
                ...
                // ALGORITHM LINE 20 //
                ///////////////////////
                cout << "Back into Queue: "<< it.first->getName() <<" Dependency Conflict"<< endl;
    ...
    bool RationalIIModuloSDCScheduler::dependencyConflictForTwoInstructions(Vertex *i, const int &newStartTime_i, const int &newStartTime_j,
    ...
    void RationalIIModuloSDCScheduler::setMRT(RationalIIModuloSDCSchedulerMRT &mrt, ResourceModel &resourceModel, std::vector<int> &initiationIntervals, int samples, int modulo, bool quiet) {
        if(!quiet) {
        ...
        for(auto res : rm.Resources()) {
            ...
            if(numberOfVertices==1) {
                ...
                for(unsigned int i=0; i<modulo; ++i) {
                    ...
                    if(!quiet) std::cout << "specified MRT column height1 = " << mrt.getHeight(resourceMap[res],i) << " for resource " << resourceMap[res]->getName() << " at column " << i << std::endl;
                }
                continue;
            }
            // handle case - resource limit modulo #samples == 0 => build trivial MRT with height=limit/#samples
            std::cout << "samples value: " << samples << endl;
            if(limit % samples == 0) {
                ...
                for(unsigned int i=0; i<modulo; ++i) {
                    ...
                    if(!quiet) std::cout << "specified MRT column height2 = " << mrt.getHeight(resourceMap[res],i) << " for resource " << resourceMap[res]->getName() << " at column " << i << std::endl;
                }
                continue;
            }
            std::cout << "vertices counter: " << numberOfVertices << endl;
            for(unsigned int i=0; i<modulo; ++i) {
                ...
                if(!quiet) std::cout << "specified MRT column height3= " << mrt.getHeight(resourceMap[res],i) << " for resource " << resourceMap[res]->getName() << " at column " << i << std::endl;
            }
        }
    }
    ...
    RationalIIModuloSDCSchedulerMRT::RationalIIModuloSDCSchedulerMRT(ResourceModel &rm, unsigned int II) : quiet(true), rm(&rm), II(II) {
    ...
    void RationalIIModuloSDCSchedulerMRT::specifyColumnHeight(const Resource *res, unsigned int column, unsigned int height) {
        if(this->mrt.find(res) == this->mrt.end())
    ...
    std::list<Vertex *> RationalIIModuloSDCSchedulerMRT::getResourceConflicts(Vertex *I, const int &evictTime){
        ...
        if(evictTime>=this->II)
            throw HatScheT::Exception("Invalid modulo slot requestedtest: "+to_string(evictTime)+", II="+to_string(this->II));
