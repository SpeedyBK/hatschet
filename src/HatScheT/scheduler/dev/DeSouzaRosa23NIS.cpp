...
void HatScheT::NonIterativeModuloScheduler::scheduleIteration() {
    ...
    if (custom) {
        ...
        orderedInstructions.insert(orderedInstructions.end(), orderedForwardInstructions.begin(),
                                   orderedForwardInstructions.end());
    } else {
            ...
            std::vector<CycleVec> cycles;
            {
                std::vector<Edge *> backEdges;
                {
                    ...
                    for (auto backEdge: backEdges) {
                        vector<CycleVec> backEdgeCycles = getCycleAndSlacks(&backEdge->getVertexDst(),
                                                                            &backEdge->getVertexSrc(), 0,
                                                                            backEdge->getDistance() * curII, vMap);
        ...
        for (auto edge: edges) {
            auto isSameVertex1 = [&edge](Vertex *v) {
              return edge->getVertexSrc().getId() == v->getId();
            }; //TODO a bit ugly?
            if (std::find_if(orderedInstructions2.begin(), orderedInstructions2.end(), isSameVertex1) ==
                orderedInstructions2.end()) {
                orderedInstructions2.push_back(&edge->getVertexSrc());
            }
            auto isSameVertex2 = [&edge](Vertex *v) {
              return edge->getVertexDst().getId() == v->getId();
            }; //TODO a bit ugly?
            if (std::find_if(orderedInstructions2.begin(), orderedInstructions2.end(), isSameVertex2) ==
                orderedInstructions2.end()) {
    ...
    for (auto inst: orderedInstructions) {
        ...
        if (this->resourceModel.getResource(inst)->isUnlimited()) {
            ...
            while (congruencies.count(inst) == 0) {
                if (Utility::resourceAvailable(congruencies, &this->resourceModel,
                                               this->resourceModel.getResource(inst), inst, m, curII)) {
    ...
    for (auto edge: this->g.Edges()) {
        double val1 = static_cast<double>(-(getLatency(&edge->getVertexSrc()) + edge->getDelay()) +
                                          edge->getDistance() * curII - (congruencies.at(&edge->getVertexSrc()) -
                                                                         congruencies.at(&edge->getVertexDst())));
    ...
    for (auto &helper: helpers) {
        ...
        this->startTimes.insert({&this->g.getVertexById(helper.first), (helper.second - smallest) * curII +
                                                                       congruencies.at(
                                                                           &this->g.getVertexById(helper.first))});
    }
    ...
    for (auto inst: this->g.Vertices()) {
        if (this->startTimes.count(inst) == 0) {
            for (int i = congruencies.at(inst); i < curII; i++) {
                if (this->startTimes.count(inst) == 0) {
                    if (Utility::resourceAvailable(this->startTimes, &this->resourceModel,
                                                   this->resourceModel.getResource(inst), inst, i)) {
...
void HatScheT::NonIterativeModuloScheduler::propagateIncrement(std::map<Vertex *, int> &increments, Vertex *currentInst,
                                                               int increment) {
...
std::vector<HatScheT::NonIterativeModuloScheduler::CycleVec>
HatScheT::NonIterativeModuloScheduler::getCycleAndSlacks(Vertex *currentInst, Vertex *backEdgeInst, int partialSlack,
                                                         int backEdgeDistance, std::map<Vertex *, bool> &vMap) {
    ...
    for (auto edge: this->g.getOutgoingEdges(currentInst)) {
        ...
        if (edge->getVertexDst().getId() == backEdgeInst->getId()) {
            ...
            if (inserted || !vMap.at(currentInst)) {
                ...
                cycles = getCycleAndSlacks(&edge->getVertexDst(), backEdgeInst,
                                           partialSlack + getLatency(currentInst) + edge->getDelay(), backEdgeDistance,
                                           vMap);
...
std::vector<HatScheT::NonIterativeModuloScheduler::PathVec>
HatScheT::NonIterativeModuloScheduler::getBackwardsPathsAndLengths(Vertex *currentInst, int partialLength,
                                                                   std::map<Vertex *, bool> &vMap) {
    std::vector<PathVec> paths;
    ...
    for (auto edge: this->g.getIncomingEdges(currentInst)) {
        ...
        if (isSourceForCurrentLoop(edge->getVertexSrc())) {
            ...
            if (inserted || !vMap.at(currentInst)) {
                ...
                tmpPaths = getBackwardsPathsAndLengths(&edge->getVertexSrc(),
                                                       partialLength + getLatency(currentInst) + edge->getDelay(),
                                                       vMap);
                vMap.at(currentInst) = false;
            }
...
std::vector<HatScheT::NonIterativeModuloScheduler::PathVec>
HatScheT::NonIterativeModuloScheduler::getForwardsPathsAndLengths(Vertex *currentInst, int partialLength,
                                                                  std::map<Vertex *, bool> &vMap) {
    std::vector<PathVec> paths;
    ...
    for (auto edge: this->g.getOutgoingEdges(currentInst)) {
        ...
        if (isSinkForCurrentLoop(edge->getVertexDst())) {
            ...
            if (inserted || !vMap.at(currentInst)) {
                ...
                tmpPaths = getForwardsPathsAndLengths(&edge->getVertexDst(),
                                                      partialLength + getLatency(currentInst) + edge->getDelay(), vMap);
                vMap.at(currentInst) = false;
                for (auto &path: tmpPaths) {
                    path.push_back(currentInst);
                }
...
std::vector<HatScheT::Vertex *>
HatScheT::NonIterativeModuloScheduler::establishTopologicalOrderForCycle(Vertex *currentInst,
...
std::vector<HatScheT::Vertex *>
HatScheT::NonIterativeModuloScheduler::establishTopologicalOrderForPath(Vertex *currentInst,
                                                                        std::map<Vertex *, OrderedWeight> &wMap) {
...
std::vector<HatScheT::NonIterativeModuloScheduler::PathVec>
HatScheT::NonIterativeModuloScheduler::getPathsAndCycles(Vertex *currentInst, int currentLength,
    ...
    for (auto edge: this->g.getOutgoingEdges(currentInst)) {
        ...
        if (isSinkForCurrentLoop(edge->getVertexDst())) {
                ...
                if (suc == vMap.end() || !suc->second) {
                    ...
                    for (auto &keyVal: currentCyclePaths) {
                        if (keyVal.first->getVertexSrc().getId() == edge->getVertexDst().getId()) {
                            ...
                            tmpVec.length =
                                getLatency(currentInst) + edge->getDelay() + getLatency(&edge->getVertexDst());
            ...
            if (inserted || !vMap.at(currentInst)) {
                ...
                tmpPaths = getPathsAndCycles(&edge->getVertexDst(),
                                             currentLength + getLatency(currentInst) + edge->getDelay(),
                                             currentCyclePaths, vMap);
                vMap.at(currentInst) = false;
