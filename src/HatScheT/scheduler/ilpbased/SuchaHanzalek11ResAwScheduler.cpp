  ...
  for (int candII = minII; candII <= maxII; ++candII) {
    ...
    if (feasible) {
      ...
      for (auto entry : m) {
        ...
        if (! p->isUnlimited())
          resourceModel.getResource(p->getName())->setLimit(numAllocated); // XXX: is this the right thing to do?
  ...
  for(auto it = this->resourceModel.resourcesBegin(); it != resourceModel.resourcesEnd(); ++it){
    ...
    for(auto it2 = minCosts.begin(); it2 != minCosts.end(); ++it2){
      if(this->target.getElement(it2->first) - minCosts[it2->first] < 0)
        throw Exception("SH11RA.compute_m_max: Error negative space detected for hardware element " + it2->first);
    ...
    for(auto it2 = r->getHardwareCosts().begin(); it2 != r->getHardwareCosts().end(); ++it2){
      ...
      if(unitsFit < 1){
        ...
        cout << "SH11RA.compute_m_max: Error " << remainingSpace[costName] << " / " << resCost << " < 1 " << endl;
        throw Exception("SH11RA.compute_m_max: Error no space left allocating one hardware unit of resource " + r->getName());
      }
      if(Ak == 0) Ak = unitsFit;
      else if(unitsFit < Ak) Ak = unitsFit;
    }
    if(Ak == 0) throw Exception("SH11RA.compute_m_max: Error maximal resource allocation of 0 determined: " + r->getName());
