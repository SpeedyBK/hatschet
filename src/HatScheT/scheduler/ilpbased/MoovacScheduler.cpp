...
namespace HatScheT
{
	...
	void MoovacScheduler::setObjective()
	{
		...
		{
			cout << "MoovacScheduler: Latencyminimize objective Disabled!" << endl;
		}
	}
	...
	void MoovacScheduler::scheduleIteration() {
		if(!this->quiet)
		{
		...
		this->secondObjectiveOptimal = (stat == ScaLP::status::OPTIMAL);
		...
		if(this->scheduleFound)
		{
			...
			if(!this->quiet)
			{
				if (this->optimalResult) cout << "Found optimal solution for II: " << this->II << endl;
